  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '22px 32px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(135deg, #fff 60%, var(--indigo-lt) 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div>
          <h1 className="font-display font-bold text-xl" style={{ fontSize: 24, margin: 0, lineHeight: 1 }}>Tableau de bord</h1>
          <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--text-3)', margin: '5px 0 0' }}>
            Approche <strong style={{ color: 'var(--indigo)' }}>{approach}</strong> · {dataset.length} paires Q/R · {APPROACH_CONFIG[approach]?.model}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={handleRun} disabled={loading} className="btn-primary">
            <Play size={14} /> {loading ? 'Évaluation…' : 'Lancer le benchmark'}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        {!results && !loading && (
          <div className="card" style={{ padding: '64px 0', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', margin: 0 }}>Prêt à évaluer</p>
            <p style={{ fontSize: 13, color: 'var(--text-3)', margin: '6px 0 0' }}>
              {dataset.length} paires Q/R chargées · Approche {approach} sélectionnée
            </p>
          </div>
        )}
        ...
        {loading && (
          <div className="card" style={{ padding: '64px 0', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--text-2)', margin: 0 }}>Calcul BLEU · ROUGE-L · Métriques de performance…</p>
          </div>
        )}
        ...
        {results && (
          <>
            {/* Quality KPIs */}
            <div>
              <SectionLabel>Qualité des réponses</SectionLabel>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                <KpiCard label="BLEU" value={results.bleu} unit="%" icon={TrendingUp} color="var(--teal)" good={0.5} warn={0.3} />
                <KpiCard label="ROUGE-L" value={results.rouge_l} unit="%" icon={TrendingUp} color="var(--indigo)" good={0.5} warn={0.3} />
                <KpiCard label="Pertinence" value={results.contextual_relevance_rate} unit="%" icon={ShieldCheck} color="var(--amber)" good={0.8} warn={0.5} />
                <KpiCard label="Précision langue" value={results.lang_accuracy} unit="%" icon={ShieldCheck} color="var(--teal)" good={0.9} warn={0.7} />
              </div>
            </div>

            {/* Performance KPIs */}
            <div>
              <SectionLabel>Performance système</SectionLabel>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                <KpiCard label="TTFT" value={results.ttft_ms} unit="ms" icon={Clock} color="var(--indigo)" good={1000} warn={3000} />
                <KpiCard label="Latence moy." value={results.avg_latency_ms} unit="ms" icon={Clock} color="var(--amber)" good={2000} warn={5000} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <div className="card" style={{ padding: '20px 24px' }}>
                <SectionLabel>Radar qualité</SectionLabel>
              ...
              <div className="card" style={{ padding: '20px 24px' }}>
                <SectionLabel>Latence (ms)</SectionLabel>
                <ResponsiveContainer width="100%" height={220}>
            ...
            {metrics && (
              <div>
                <SectionLabel>Fiabilité système</SectionLabel>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14 }}>
                  <KpiCard label="Uptime" value={metrics.uptime_percent/100} unit="%" icon={ShieldCheck} color="var(--teal)" good={99} warn={95} />
                  <KpiCard label="Rate limits" value={metrics.rate_limit_hits} unit="" icon={Zap} color="var(--amber)" good={0} warn={5} />
                  <KpiCard label="Cold start" value={metrics.cold_start_ms} unit="ms" icon={Clock} color="var(--indigo)" good={5000} warn={30000} />
                  <KpiCard label="Concurrence" value={metrics.concurrent_requests_handled} unit="" icon={TrendingUp} color="var(--teal)" good={5} warn={2} />
                  <KpiCard label="Coût total" value={metrics.cost_eur} unit="€" icon={Zap} color="var(--amber)" good={0} warn={0} />
                </div>
              </div>
            )}
          </>
